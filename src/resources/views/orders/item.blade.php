...
@section('admin-content')
	...
	<div class="admin-content">
		...
		<div class="one-box">
			<div class="background-image" style="background-image: url({{ !empty($orderItem->product->thumb) ? 'uploads/'.$orderItem->product->thumb : 'images/no-image.svg' }})"></div>
		</div>
		...
		<ul class="border">
			@foreach ($orderItem->variations as &$variation)
				...
				@if (!empty($variation->productVariation->image)) <img src="uploads/{{$variation->productVariation->image}}"> @endif
				<b>
