...
@section('admin-content')
	...
	<div class="admin-content">
		<form action="galleries/images/update-element/{{ $element->id }}" method="post" enctype="multipart/form-data">
			...
			<label>Title</label>
			<input type="text" name="title" placeholder="Page title" value="{{ $element->title ?? old('title') }}">
			...
			@elseif ($element->page_element_type_id == 2 || old('page_element_type_id') == 2)
				...
				<label>Content</label>
				<textarea name="content" class="htmlEditor" data-page-name="image_element" data-page-id="{{ $element->id }}" id="editor-{{ $element->id }}" rows="5" placeholder="Content">{{ $element->content ?? old('content') }}</textarea>
